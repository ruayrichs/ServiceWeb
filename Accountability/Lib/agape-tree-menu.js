...
$.fn.aGapeTreeMenu = function (data) {
    ...
    var CurrentDate = data.CurrentDate == undefined ? "" : data.CurrentDate;
    var disPlaySatatus = data.disPlaySatatus == undefined ? false : data.disPlaySatatus;
    ...
    var removeSwitchEmptyFolder = data.removeSwitchEmptyFolder == undefined ? false : data.removeSwitchEmptyFolder;
    ...
    var imgShare = $("<i/>", {
        ...
        click: function () {
            if (onShare != undefined) {
                ...
                $("#" + TreeID + " .agape-tree-menu-checkbox-v").each(function () {
                ...
                $("#" + TreeID + " .agape-tree-menu-checkbox-s").each(function () {
                    if ($(this).prop("checked")) {
                        sharedatas.push({
                            id: $(this).parent().find(".agape-hidden-node-id").html(),
                            name: $(this).parent().find(".agape-hidden-node-name").html(),
    ...
    $(RootShowName).bind("mousedown", {
        ...
        if (e.data.type != "e" && e.button == 2) {
            $(this).appendRightClickCommand(e, undefined, onNewFolder, undefined, TreeID);
        }
    });
    ...
    $(liRoot).aGapeAppendInnerTree({
        ...
        chart: "",
        name: rootText
    }, datas, onClick, "", onRename, onDelete, onWorkGroup, focusChart, onlyFolder, focusOnClick, selecting, emptyFolder, onNewFolder, focusColor, rootCode, rootText, PossibleEntry, disPlaySatatus, CurrentDate, TreeID);
    ...
    $(RootShowName).click(function (e) {
        ...
        if (focusColor) {
            $("#" + TreeID + " .agape-show-node-name").css("color", "#000");
            $("#" + TreeID + " .agape-show-node-name").css("background-color", "#fff");
            $("#" + TreeID + " .agape-show-node-name").css("padding", "0");
            $("#" + TreeID + " .agape-show-node-name").css("border-radius", "0");
    ...
    $("#" + TreeID + " .agape-tree-menu-checkbox-f").click(function () {
        $(this).parent().find(".agape-tree-menu-checkbox").prop("checked", $(this).prop("checked"));
    });


    $("#" + TreeID + " .agape-tree-li").each(function () {
    ...
    if (moveItem) {
        $("#" + TreeID + " .agape-tree-menu-child").sortable({
            ...
            over: function (event, ui) {
                $("#" + TreeID + " .agape-tree-li,.agape-tree-li-root").css({
                    border: "none",
                    paddingBottom: 0
                });
            ...
            stop: function (event, ui) {
                $("#" + TreeID + " .agape-tree-menu-empty-folder").hide();
    ...
    $(document).click(function (e) {
        $("#" + TreeID + " .agape-tree-menu-right-click").remove();
    });

    if (removeSwitchEmptyFolder) {
        $("#" + TreeID + " .agape-tree-li.agape-tree-menu-folder").each(function () {
            if ($(this).find(".agape-tree-li.agape-tree-menu-folder").length == 0) {
                var elmSwitch = $(this).find(".agape-tree-menu-switch");
                elmSwitch.removeClass("fa");
                elmSwitch.removeClass("fa-minus-square-o");
                elmSwitch.css({
                    marginRight: "19px"
                });
            }
...
$.fn.aGapeAppendInnerTree = function (parentObject, treeObject, onClick, oldParentNode, onRename, onDelete, onWorkGroup,
    focusChart, onlyFolder, focusOnClick, selecting, emptyFolder, onNewFolder, focusColor, rootCode, rootText,
    PossibleEntry, disPlaySatatus, CurrentDate, TreeID) {
    ...
    for (var i = 0; i < treeObject.length; i++) {
        ...
        var arrDataParam = jQuery.param(treeObject[i]).split("&");
        for (var j = 0; j < arrDataParam.length; j++) {
            if (arrDataParam[j].indexOf("tree") > -1) {
                continue;
            }
            var arrAttr = arrDataParam[j].split("=");
            var param = arrAttr[0];
            var value = arrAttr[1];
            innerLI.attr("data-param-" + param, value);
        }
        ...
        var StatusDisplay = "";
        if (disPlaySatatus) {
            //, disPlaySatatus, CurrentDate

            if (treeObject[i].StatusCode == '002') {
                //ปิดโครงการ
                StatusDisplay = "<i class='fa fa-check-circle-o' style='color: #28b120 !important;'></i>";
            } else if (treeObject[i].StatusCode == '004') {
                //ถูกยกเลิก/ไม่เสร็จสิ้น
                StatusDisplay = "<i class='fa fa-times-circle-o' style='color: #e8b009 !important;'></i>";
            } else if (treeObject[i].StatusCode == '003') {
                //ล่าช้า
                StatusDisplay = "<i class='fa fa-clock-o' style='color: #f00 !important;'></i>";
            } else {
                //ยังไม่เริ่มดำเนินการ & ดำเนินการ
                if (treeObject[i].StatusCode == '000') {
                    //ยังไม่เริ่มดำเนินการ
                    //StatusDisplay = "<i class='fa fa-play-circle' style='color: #000 !important;'></i>";
                    StatusDisplay = "";
                }
                if (treeObject[i].StatusCode == '001') {
                    //ดำเนินการ
                    StatusDisplay = "<i class='fa fa-play-circle' style='color: #08c !important;'></i>";
                }

                if (parseInt(CurrentDate) > parseInt(treeObject[i].EndDate)) {
                    //ล่าช้า
                    StatusDisplay = "<i class='fa fa-clock-o' style='color: #f00 !important;'></i>";
                }
            }
        ...
        var showName = $("<span/>", {
            ...
            html: "<span class='node-possible-entry-name' style='color:#aaa'>" + (posibleNode.length > 0 ? ("[" + posibleNode[0].Name + "] ") : "") + StatusDisplay + "</span>" + name,
            css: {
                cursor: "pointer"
            },
            title: (posibleNode.length > 0 ? ("[" + posibleNode[0].Name + "] ") : "") + name
        });
        ...
        $(showName).bind("mousedown", {
            ...
            if (e.data.type != "e" && e.button == 2) {
                $(this).appendRightClickCommand(e, onRename, onNewFolder, onDelete, onWorkGroup, TreeID);
            }
        });
        ...
        if (type == "f") {
            ...
            if (treeObject[i].tree != undefined && treeObject[i].tree.length > 0) {
                $(innerLI).aGapeAppendInnerTree({
                    ...
                    chart: chart,
                    name: name
                }, treeObject[i].tree, onClick, id, onRename, onDelete, onWorkGroup, focusChart, onlyFolder, focusOnClick, selecting, emptyFolder, onNewFolder, focusColor, rootCode, rootText, PossibleEntry, disPlaySatatus, CurrentDate, TreeID)
            }
            else {
                if (emptyFolder) {
                    $(innerLI).aGapeAppendInnerTree({
                        ...
                        type: "e",
                        tree: []
                    }], undefined, id, onRename, onDelete, onWorkGroup, focusChart, onlyFolder, focusOnClick, selecting, emptyFolder, onNewFolder, focusColor, rootCode, rootText, PossibleEntry, disPlaySatatus, CurrentDate, TreeID);
                }
            }
...
$.fn.appendRightClickCommand = function (e, onRename, onNewFolder, onDelete, onWorkGroup, TreeID) {
...
$.fn.FocusOnClick = function (TreeID) {
    $("#" + TreeID + " .agape-show-node-name").css({
        background: "",
        color: "#000",
        padding: 0
    });
