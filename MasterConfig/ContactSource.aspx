<%@ Page Title="" Language="C#" MasterPageFile="~/MasterConfig/MasterPage/MasterPageConfig.master" AutoEventWireup="true" CodeBehind="ContactSource.aspx.cs" Inherits="ServiceWeb.MasterConfig.ContactSource" %>
...
<asp:Content ID="Content1" ContentPlaceHolderID="ContentPlaceHolderConfig" runat="server">
    <script>
        function webOnLoad() {
            ...
            //set new active
            document.getElementById("nav-menu-contract-source").className = "nav-link active";
        };
     ...
     <nav class="navbar nav-header-action sticky-top bg-white" style="margin-left: 1px; margin-right: 1px;">
        <div class="pull-left">
            <asp:UpdatePanel ID="udpnAction" runat="server" UpdateMode="Conditional">
                <ContentTemplate>
                    <button type="button" class="btn btn-primary mb-1 AUTH_MODIFY" onclick="openCreate();"><i class="fa fa-plus-circle"></i>&nbsp;&nbsp;New Contact Source</button>
                </ContentTemplate>
            </asp:UpdatePanel>
        </div>
    </nav>
        ...
        <div class="col">
            <div class="card shadow">
                <div class="card-header">
                    <h5 class="mb-0">Contact Source</h5>
                </div>
                <div class="card-body">
                                ...
                                <table id="tableItems" class="table table-bordered table-striped table-hover table-sm">
                                    <thead>
                                        <tr>
                                            ...
                                            <th class="text-nowrap">Problem Source</th>
                                                ...
                                                <tr>
                                                    <td class="text-nowrap text-center align-middle">
                                                        <asp:UpdatePanel runat="server" UpdateMode="Conditional">
                                                            <ContentTemplate>
                                                                <i class="fa fa-edit fa-lg text-dark mx-1 AUTH_MODIFY" title="Edit" onclick="openEdit('<%# Eval("BUSINESSOBJECT") %>','<%# Eval("GROUPCODE") %>','<%# Eval("TYPECODE") %>','<%# Eval("SOURCECODE") %>','<%# Eval("AREACODE") %>');"></i>                                                        
                                                                <i class="fa fa-times-circle-o fa-lg text-danger mr-1 AUTH_MODIFY" title="Delete" onclick="confirmDelete(this);"></i>
                                                                <asp:Button ID="btnDelete" runat="server" CssClass="d-none AUTH_MODIFY" OnClick="btnDelete_Click" CommandArgument='<%# Eval("AREACODE") %>' />
                                                                ...
                                                                <asp:HiddenField ID="hdfTypeCode_" Value='<%# Eval("TYPECODE") %>' runat="server" />
                                                                <asp:HiddenField ID="hdfSourceCode_" Value='<%# Eval("SOURCECODE") %>' runat="server" />
                                                    ...
                                                    <td class="text-nowrap">
                                                     ...
                                                     <td class="text-nowrap">
                                                        <%# Eval("SOURCECODE") +" : " + Eval("SOURCENAME") %>
                                                    </td>
                                                    <td class="text-nowrap">
                                                        <%# Eval("AREACODE") +" : " + Eval("AREANAME") %>
                                                    </td>
                                                    <td class="text-nowrap">
                                                        <%# Eval("OwnerGroupName") %>
                                                    </td>
        ...
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="modal-header">Contact Source</h5>
                    ...
                    <asp:UpdatePanel ID="udpn" runat="server" UpdateMode="Conditional">
                        <ContentTemplate>
                            ...
                            <div class="form-group" runat="server">
                                ...
                                <asp:DropDownList ID="ddlProblemGroup" onchange="$('#btnSetGroupType').click();" CssClass="form-control form-control-sm required" runat="server">
                                </asp:DropDownList>
                                <asp:Button ID="btnSetGroupType" ClientIDMode="Static" OnClick="btnSetGroupType_Click" OnClientClick="inactiveRequireField();AGLoading(true);" CssClass="d-none" runat="server" />
                            </div>
                            <div class="form-group" runat="server">
                                <label>Problem Type</label>
                                <asp:DropDownList ID="ddlProblemType" onchange="$('#btnSetGroupSource').click();" CssClass="form-control form-control-sm required" runat="server">
                                </asp:DropDownList>
                                <asp:Button ID="btnSetGroupSource" ClientIDMode="Static" OnClick="btnSetGroupSource_Click" OnClientClick="inactiveRequireField();AGLoading(true);" CssClass="d-none" runat="server" />
                            </div>
                            <div class="form-group" runat="server">
                                <label>Problem Source</label>
                                <asp:DropDownList ID="ddlProblemSource" CssClass="form-control form-control-sm required" runat="server">
                                </asp:DropDownList>
                            </div>
                            <div class="form-group">
                                <label>Code</label>
                            ...
                            <asp:HiddenField ID="hdfTypeCode" runat="server" />   
                            <asp:HiddenField ID="hdfSourceCode" runat="server" /> 
    ...
    <script>
        ...
        function openEdit(business, group, typecode, sourcecode, code) {
            inactiveRequireField();
            $("#<%= hdfBusiness.ClientID %>").val(business);
            $("#<%= hdfGropCode.ClientID %>").val(group);
            $("#<%= hdfTypeCode.ClientID %>").val(typecode);
            $("#<%= hdfSourceCode.ClientID %>").val(sourcecode);
        ...
        function openModal(mode) {
            ...
            $("#modal-header").html(mode + " Contact Source");
