...
$.fn.aGapeTreeMenu = function (data) {
    ...
    var IsHighlight = data.IsHighlight == undefined ? false : data.IsHighlight;
    ...
    $(liRoot).aGapeAppendInnerTree({
        ...
        chart: "",
        name: rootText
    }, datas, onClick, "", onRename, onDelete, onWorkGroup, focusChart, onlyFolder, focusOnClick, selecting, emptyFolder, onNewFolder, focusColor, rootCode, rootText, PossibleEntry, IsHighlight);
...
$.fn.aGapeAppendInnerTree = function (parentObject, treeObject, onClick, oldParentNode,
    onRename, onDelete, onWorkGroup, focusChart, onlyFolder, focusOnClick, selecting,
    emptyFolder, onNewFolder, focusColor, rootCode, rootText, PossibleEntry, IsHighlight) {
    ...
    for (var i = 0; i < treeObject.length; i++) {
        ...
        var IsNew = treeObject[i].IsNewItem == undefined ? false : treeObject[i].IsNewItem;
        IsHighlight = IsHighlight == undefined ? false : IsHighlight;
        ...
        var showName = $("<span/>", {
            ...
            html: "<span class='node-possible-entry-name' style='color:#aaa'>" +
                (posibleNode.length > 0 ? ("[" + posibleNode[0].Name + "] ") : "") +
                "</span>" + (IsNew && IsHighlight ? "<span class='label label-warning blink-new' style='margin-right: 3px;'>New</span>" : "") + name,
            css: {
                cursor: "pointer"
            },
            ondrop: "drop(event, this)",
        ...
        if (type == "f") {
            ...
            if (treeObject[i].tree != undefined && treeObject[i].tree.length > 0) {
                $(innerLI).aGapeAppendInnerTree({
                    ...
                    chart: chart,
                    name: name
                }, treeObject[i].tree, onClick, id, onRename, onDelete, onWorkGroup, focusChart, onlyFolder, focusOnClick, selecting, emptyFolder, onNewFolder, focusColor, rootCode, rootText, PossibleEntry, IsHighlight)
            }
            else {
                if (emptyFolder) {
                    $(innerLI).aGapeAppendInnerTree({
                        ...
                        type: "e",
                        tree: []
                    }], undefined, id, onRename, onDelete, onWorkGroup, focusChart, onlyFolder, focusOnClick, selecting, emptyFolder, onNewFolder, focusColor, rootCode, rootText, PossibleEntry, IsHighlight);
                }
            }
